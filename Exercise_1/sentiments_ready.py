...
# Replacing the values to ease understanding -> data['target'] = data['target'].replace(1,0) -> data['target'] = data['target'].replace(2,1)
# data.loc[:, 'target'] = data['target'].replace(1, 0)
# data.loc[:, 'target'] = data['target'].replace(2, 1)
...
dataset['text'] = dataset['text'].apply(lambda x: cleaning_txt(x))
dataset['text'].head()
